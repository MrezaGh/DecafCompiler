...
do
	...
	do
		...
		if command -v python3; then
			python3 -m src.main -i "$folder/$filelist" -o "$folder/$output_asm"
		else
			python -m src.main -i "$folder/$filelist" -o "$folder/$output_asm"
		fi
		if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			...
			if command -v python3; then
				python3 ./src/comp.py -a "$OUTPUT_DIRECTORY$folder/$output_filename" -b "$TEST_DIRECTORY$folder/$output_filename" -o "$REPORT_DIRECTORY$folder/$report_filename"
			else
				python ./src/comp.py -a "$OUTPUT_DIRECTORY$folder/$output_filename" -b "$TEST_DIRECTORY$folder/$output_filename" -o "$REPORT_DIRECTORY$folder/$report_filename"
			fi
			if [[ $? = 0 ]]; then
				((NUMBER_OF_PASSED++))
				echo "++++ test passed"
